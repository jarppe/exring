(ns exring.http-response)

; Copied from https://github.com/metosin/ring-http-response/blob/master/src/ring/util/http_response.clj
; Removed potemkin stuff because cljs
...
(defn throw!
  ...
  [response]
  {:pre (map? response)}
  (throw (ex-info (str "HTTP " (:status response)) {:type     :ring.util.http-response/response
                                                    :response response})))
